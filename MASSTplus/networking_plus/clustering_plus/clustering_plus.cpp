        ...
        if (file_ext == "mgf")
        {
            ...
            while (getline(f, a))
            {
                if (a == "BEGIN IONS")
                {
                    ...
                    verification += 1;
                    double pepmass = -1;
                    double rtinsec = -1; // Default to 0.0 instead of -1
                    int charge = 0;
                    while (getline(f, a))
                    {
                        ...
                        if (a.rfind("CHARGE=", 0) == 0)
                        {
                            string charge_str = a.substr(7);
                            charge = stoi(charge_str);
                        }
...
void write_cluster_centers(spectratoinfo_t &spectrainfo_all, vector<int> &scans, string cluster_center_res)
{
    ...
    for (int i = 0; i < scans.size(); i++)
    {
        ...
        int charge = spectra_general_info[center_scan].charge; // Retrieve charge

        cluster_center_writer << "BEGIN IONS" << endl;
        cluster_center_writer << "PEPMASS=" << pepmass << endl;
        cluster_center_writer << "CHARGE=" << charge << "+" << endl; // Add charge information
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    cluster_outputf << "cluster_idx" << "\t" << "average mz" << "\t" << "average RT" << "\t" << "average charge" << "\t" << "num spectra" << endl;
    // outf << "cluster_idx" << "\t" << "scan" << "\t" << "mz" << "\t" << "RTINSECONDS" << "\t" << "charge" << "\t" << "index in file" << "\t" << "source filename" << endl;
    outf << "precursor_charge" << "\t" << "precursor_mz" << "\t" << "identifier" << "\t" << "scan" << "\t" << "retention_time" << "\t" << "cluster_idex" << "\t" << "index in file" << endl;
    ...
    for (int cluster_idx = 0; cluster_idx < cluster_content.size(); cluster_idx++)
    {
        ...
        for (int content_idx = 0; content_idx < content_tmp.size(); content_idx++)
        {
            ...
            int charge = spectra_general[query_scan].charge; // Extract charge info

            pepmass_sum += mz;
            RT_sum += RT;
            charge_sum += charge; // Add charge to compute average later

            string src_filename = scan_file_src[query_scan];
            int local_index = query_scan - file_start_scan[src_filename];

            // outf << cluster_idx << "\t" << query_scan << "\t" << mz << "\t" << RT << "\t" << charge << "\t" << local_index << "\t" << src_filename << endl;
            outf << charge << "\t" << mz << "\t" << src_filename << "\t" << query_scan << "\t" << RT << "\t" << cluster_idx << "\t" << local_index << endl;
        }
        double pepmass_sum_avg = pepmass_sum / num_spectra;
        double RT_avg = RT_sum / num_spectra;
        double charge_avg = (double)charge_sum / num_spectra;
        cluster_outputf << cluster_idx << "\t" << pepmass_sum_avg << "\t" << RT_avg << "\t" << charge_avg << "\t" << num_spectra << endl;
    }
