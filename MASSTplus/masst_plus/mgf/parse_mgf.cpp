	...
	: filter_peaks(filter_peaks), pos_bin_size(pos_bin_size), mass_bin_size(mass_bin_size)
{
	...
	if (!use_stream)
	{
		...
		else
		{
			...
			std::cout << "Initializing ParseMGF for file: " << spectra_file << std::endl;
...
int ParseMGF::parseNextFile(std::ofstream &log_file)
{
	...
	if (file.is_open())
	{
		...
		while (std::getline(file, line))
		{
			...
			{
				std::cout << "Found BEGIN IONS in file: " << curr_file_path << std::endl;
				...
				if (parse_status < 0)
				{
					std::cout << "Error: parseSpectrum() failed for spectrum #" << cur_can_num << std::endl;
					file.close();
					return parse_status;
				}

				std::cout << "Parsed spectrum #" << cur_can_num << ", Title: " << spectrum.title
						  << ", Mass: " << spectrum.mass << ", Charge: " << spectrum.charge
						  << ", Peaks: " << spectrum.peaks.size() << std::endl;
