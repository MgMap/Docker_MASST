...
void getTopMatches(std::vector<Match> &matches, std::vector<Match> &top_matches, int num_peaks)
{
	...
	while (top_matches.size() < max_matches && matches.size() > 0)
	{
		...
		auto iter_a = std::find_if(top_matches.begin(), top_matches.end(), [&](Match &m)
								   { return m.a == n.a; });
		auto iter_b = std::find_if(top_matches.begin(), top_matches.end(), [&](Match &m)
								   { return m.b == n.b; });
...
int main(int argc, char **argv)
{
	...
	std::cout << "Query file loaded: " << query_file_name << std::endl;
	std::cout << "Number of spectra in query: " << queryMGF.curr_spectra.size() << std::endl;
