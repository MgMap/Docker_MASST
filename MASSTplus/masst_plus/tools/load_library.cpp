	...
	else
	{
		while (!libraryMGF.hasFinishedParsing())
		{
			...
			else
			{
				...
				total_peaks_count += num_peaks;
				log_file << std::endl
						 << "Inserted " << libraryMGF.curr_spectra.size() << " spectra (" << spectra_count << " total), " << num_peaks << " peaks (" << total_peaks_count << " total)." << std::endl;
				auto current_time = std::chrono::high_resolution_clock::now();
				auto time_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - loading_start);
				log_file << "Time elapsed: " << time_elapsed.count() << " ms." << std::endl
						 << std::endl;
			}
		}
	}
